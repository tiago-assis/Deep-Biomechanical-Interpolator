...
| `-i`, <br> `--init_disp` | `str` | `None` | Path to the initial displacement field (`.h5`/`.hdf5` or `.npz`). If not provided, it will be generated by interpolating the provided keypoints. |
...
| `-m`, <br> `--interp_mode` | `str` | `'tps'` | Interpolation mode used to generate the initial field (`tps` or `linear`). |
